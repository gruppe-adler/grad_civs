...

    {
        ...
        private _text = if (_civAngle < _fov) then {
            switch (GVAR(civStateFormat)) do {
                ...
                case 11: {format["%1 | houseworkanim: %2, animState %3, gestureState %4, pose %5", _x, _x getVariable [QEGVAR(residents,housework_anim), ""], animState _x, gestureState _x, pose _x]};
                default {"<empty>"};
            };
        } else {"o"};
