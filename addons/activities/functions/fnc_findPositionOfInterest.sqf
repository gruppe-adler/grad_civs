...
private _nearestBuilding = if (_buildings isNotEqualTo []) then {_buildings select 0} else {objNull};
...
private _position = if (count _buildingPositions > 0) then {selectRandom _buildingPositions} else {
    [_searchPosition,[50,100],[0,360]] call EFUNC(lifecycle,findRandomPos);
};
